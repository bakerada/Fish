...
def showResults(img_file, results, labelmap=None, threshold=None, display=None):
    ...
    for res in results:
        if 'score' in res and threshold and float(res["score"]) < threshold:
            save_file = 
            io.imsave("{}/{}/{}.png".format(save_dir,nobox,os.path.basename(res['out_file'])),img)
        label = res['label']
    ...
    if len(results) > 0 and "out_file" in results[0]:
        ...
        bbox = res['bbox']
        xmin, ymin, xmax, ymax = np.maximum(bbox[0],0),np.maximum(bbox[1],0), bbox[2],bbox[3]
        cropped = img[int(ymin):int(ymax),int(xmin):int(xmax)]
        io.imsave(results[0]["crop_file"],cropped)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--crop-dir", default="",
            help = "A directory which saves the cropped image for detection")
    ...
    if crop_dir and not os.path.exists(crop_dir):
    ...
    with open(result_file, "r") as f:
        for line in f.readlines():
            ...
            class_dir = os.path.basename(img_name).split('_')[0]
            result = dict()
            ...
            if crop_dir:
                crop_file = "{}/{}/{}.png".format(crop_dir, class_dir, os.path.basename(img_name))
